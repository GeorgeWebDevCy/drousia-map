...
// 🔐 Plugin Settings
function gn_mapbox_register_settings() {
    register_setting('gn_mapbox_settings', 'gn_mapbox_token');
    register_setting('gn_mapbox_settings', 'gn_mapbox_debug_enabled');
    ...
    add_settings_field('gn_mapbox_token', 'Mapbox Access Token', function () {
        $value = esc_attr(get_option('gn_mapbox_token', ''));
        echo '<input type="password" name="gn_mapbox_token" value="' . $value . '" style="width:400px;" autocomplete="off">';
    }, 'gn-mapbox', 'gn_mapbox_section');

    add_settings_field('gn_mapbox_debug_enabled', 'Enable Debug Mode', function () {
        $value = get_option('gn_mapbox_debug_enabled', false);
        echo '<input type="checkbox" name="gn_mapbox_debug_enabled" value="1"' . checked(1, $value, false) . '> Show debug panel on map';
    }, 'gn-mapbox', 'gn_mapbox_section');
}
add_action('admin_init', 'gn_mapbox_register_settings');
...
function gn_mapbox_settings_page() {
    echo '<div class="wrap"><h1>GN Mapbox Settings</h1><form method="post" action="options.php">';
...
function gn_enqueue_mapbox_assets() {
    if (!is_singular() && !has_shortcode(get_post()->post_content ?? '', 'gn_map')) return;
    ...
    wp_enqueue_script('gn-mapbox-init', plugin_dir_url(__FILE__) . 'js/mapbox-init.js', ['jquery', 'mapbox-gl', 'mapbox-directions', 'chartjs'], null, true);

    wp_localize_script('gn-mapbox-init', 'gnMapData', [
        'accessToken' => get_option('gn_mapbox_token'),
        'debug' => (bool) get_option('gn_mapbox_debug_enabled', false),
        'locations' => gn_get_map_locations(),
    ]);
}
add_action('wp_enqueue_scripts', 'gn_enqueue_mapbox_assets');

// 📍 Location Data (from ACF)
...
function gn_map_shortcode() {
    return '<div id="gn-mapbox-map"></div>';
}
